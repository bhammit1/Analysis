...
from tools.functions_gipps import run_gipps_GA
from tools.functions_gipps import initiate_201802Calib_gipps_summary_file
from tools.functions_gipps import append_to_201802Calib_gipps_summary_file


# Save Scheme:
# ----------------------------------------------------------------------------------------
date = '2018-03_07_gipps_indivTrips'  # Naming for output files - Cannot be with dots - must be with dashes
...
# ----------------------------------------------------------------------------------------
trip_no_for_processing = 'itsa_calibration_all.csv'
...
# ----------------------------------------------------------------------------------------
cxpb, mutpb, m_indpb, ngen, npop = 0.5, 0.2, 0.4, 60, 800
...
initiate_201802Calib_gipps_summary_file(summary_file)
...
for iterator in range(len(trip_no_list)):
    ...
    calib_score, calib_best_indiv = run_gipps_GA(cf_collections=cf_collections, cxpb=cxpb, mutpb=mutpb,
    ...
    # ----------------------------------------------------------------------------------------
    summary_file = open(os.path.join(save_path, '{}_summary_{}.csv'.format(date,trip_no_for_processing)), 'a')
    append_to_201802Calib_gipps_summary_file(file=summary_file,
                                              trip_no=trip_no_this,
                                              driver_id=driver_id,
